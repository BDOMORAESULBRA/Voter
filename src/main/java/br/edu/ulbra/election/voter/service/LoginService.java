...
public class LoginService {
	...
	@Autowired
	public LoginService(VoterRepository voterRepository, TokenRepository tokenRepository,
	...
	public LoginOutput login(LoginInput loginInput) {
		...
		if (MD5.md5(loginInput.getPassword()) == voter.getPassword()) {
			throw new GenericOutputException(INVALID_CREDENTIALS);
		}
	...
	private String generateToken(Voter voter, Date expireDate) {
		...
		return Base64.getEncoder().encodeToString(passwordEncoder.encode(openToken).getBytes());
	}
}
